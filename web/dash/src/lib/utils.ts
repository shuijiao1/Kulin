...
export function getCpuCoreCount(cpuInfo: string[] | undefined): number {
	return (cpuInfo || []).reduce((total, info) => {
		const coreMatches = [
			...info.matchAll(/(\d+)\s*(?:Virtual\s+|Physical\s+)?Cores?/gi),
		];
		if (coreMatches.length > 0) {
			return total + coreMatches.reduce((sum, match) => sum + Number(match[1]), 0);
		}

		const trailingCountMatch = info.match(/(?:^|\s)(\d+)\s*$/);
		if (trailingCountMatch) {
			return total + Number(trailingCountMatch[1]);
		}
...
export function formatCpuCores(count: number): string {
